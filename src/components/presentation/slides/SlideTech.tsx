  ...
  <SlideLayout>
    <div className="flex flex-col h-full px-[120px] py-[100px]">
      <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
        <span className="text-[22px] font-semibold uppercase tracking-[6px] text-primary/80">Технологии</span>
        <h2 className="text-[80px] font-bold mt-4">Технологический <span className="text-gradient-primary">стек</span></h2>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-8 mt-16">
        {stack.map((s, i) => (
          ...
          >
            <span className="text-[60px]">{s.icon}</span>
            <div>
              <h3 className="text-[30px] font-semibold">{s.name}</h3>
              <p className="text-[22px] text-muted-foreground">{s.desc}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      >
        <div className="card-glass rounded-3xl p-10">
          <h3 className="text-[36px] font-semibold mb-6">📐 Архитектура</h3>
