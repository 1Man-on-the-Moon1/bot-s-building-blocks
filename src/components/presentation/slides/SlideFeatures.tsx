  ...
  <SlideLayout>
    <div className="flex flex-col h-full px-[120px] py-[100px]">
      <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
        <span className="text-[22px] font-semibold uppercase tracking-[6px] text-primary/80">Возможности</span>
        <h2 className="text-[80px] font-bold mt-4 text-gradient-primary">Полный цикл знакомства</h2>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-8 mt-16 flex-1">
        {features.map((f, i) => (
          ...
          >
            <span className="text-[60px] mb-4">{f.icon}</span>
            <h3 className="text-[32px] font-semibold mb-3">{f.title}</h3>
            <p className="text-[22px] text-muted-foreground leading-relaxed flex-1">{f.desc}</p>
          </motion.div>
        ))}
      </div>
    </div>
