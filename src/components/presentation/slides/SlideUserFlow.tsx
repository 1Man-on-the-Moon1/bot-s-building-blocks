  ...
  <SlideLayout>
    <div className="flex flex-col h-full px-[120px] py-[100px]">
      <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
        <span className="text-[18px] font-semibold uppercase tracking-[6px] text-accent/80">User Flow</span>
        <h2 className="text-[64px] font-bold mt-4">Путь пользователя</h2>
      </motion.div>
      ...
      <div className="flex items-center justify-between mt-20 flex-1 relative">
        ...
        {steps.map((step, i) => (
          ...
          >
            <div className={`w-[140px] h-[140px] rounded-3xl card-glass flex items-center justify-center text-[56px] mb-8 ${step.color === 'primary' ? 'glow-primary' : 'glow-accent'}`}>
              {step.icon}
            </div>
            <span className="text-[16px] font-mono text-muted-foreground mb-2">{step.num}</span>
            <h3 className="text-[26px] font-semibold mb-3">{step.title}</h3>
            <p className="text-[17px] text-muted-foreground leading-relaxed">{step.desc}</p>
          </motion.div>
        ))}
      </div>
    </div>
